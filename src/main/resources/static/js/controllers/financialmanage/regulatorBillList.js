...
'use strict';
app.controller('regulatorBillController', ['$scope', '$http', '$modal', 'toaster', function ($scope, $http, $modal, toaster) {
    ...
    function init(){
        ...
        $http.get('regulator-orgs/collection').success(function (data){
            var allTypes = {
                id : '0',
                name : '全部'
            };
            data.data.push(allTypes);
            $scope.regulatorOrg = data.data[data.data.length -1];
            $scope.regulatorOrgs = data.data;
            toasPop();
        }).error(function(err){
            alert(err.error);
        });
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            ...
            {field: 'regulatorOrgDetails.region', displayName: '所属区域', width: '200px', cellTemplate: '<div class="ngCellText ng-scope col8 colt8" >{{row.entity.regulatorOrgDetails.region| getRegionName }}</div>'},
            {field: 'regulatorOrgDetails.name', displayName: '所属政府机构', width: '200px'},
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        var url = 'regulator-bills?page=' + page + '&size=' + pageSize;
        if ($scope.foundTime != "")
            url += "&foundTime=" + $scope.foundTime;
        if ($scope.region != "")
            url += "&region=" + $scope.region;
        if ($scope.regulatorOrg != "" && $scope.regulatorOrg != undefined)
            url += "&name=" + $scope.regulatorOrg.name;
        if ($scope.id != "")
            url += "&id=" + $scope.id;
    
        $http.get(url).success(function (pagedata) {
            $scope.regulatorBills = pagedata.data.content;
    ...
    $scope.$watch('regulatorOrg', function (newVal, oldVal) {
        if (newVal !== oldVal) {
            $scope.getPagedDataAsync($scope.pagingOptions.pageSize, $scope.pagingOptions.currentPage, $scope.filterOptions.filterText);
        }
    }, true);
    ...
    $scope.seeRowIndex = function (entity) {
        var id = entity.id;
        var rtn = $modal.open({
            templateUrl: 'tpl/financialmanage/see_regulatorBill.html',
            controller: 'seeRegulatorBillController',
            resolve: {
                regulatorBillId: function () {return id}
            }
        });
