...
app.controller('depositBillController', ['$scope', '$http', '$modal', 'toaster', function ($scope, $http, $modal, toaster) {
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            ...
            {field: 'depositAmount', displayName: '押金金额', width: '200px', cellTemplate: '<div class="ngCellText ng-scope col8 colt8" >{{row.entity.depositAmount| getAmount}}</div>'},
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        var url = 'deposit-bills?page=' + page + '&size=' + pageSize;
        if ($scope.createTime != "")
        ...
        $http.get(url).success(function (pagedata) {
            $scope.depositBills = pagedata.data.content;
    ...
    var myStyle = {
        ...
        columns: [
            ...
            {columnid:'depositAmount', title: '押金金额', width:'100px', cell: {value: function(value){return '￥'+value}}},
    ...
    $scope.exportData = function (param){
        if (param == 'all') { // 全部导出到excel表中
            var url = 'deposit-bills/collection?1=1';
            if ($scope.createTime != "")
                url += "&createTime=" + $scope.createTime;
            if ($scope.region != "" )
                url += "&region=" + $scope.region;
            ...
            alasql('SELECT * INTO XLS("押金账单.xls",?) FROM ?', [myStyle, $scope.billDatas]);
        } else {
            var array = new Array();
            array.push(param);
            alasql('SELECT * INTO XLS("押金账单.xls",?) FROM ?', [myStyle, array]); // 单个导出到excel表中
        }
    };
