...
app.controller('subscribeBillController', ['$scope', '$http', '$modal', 'toaster', function ($scope, $http, $modal, toaster) {
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            ...
            {field: 'bikeOwnerNumber', displayName: '所属车主', width: '150px'},
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        var url = 'subscribe-bills?page=' + page + '&size=' + pageSize;
        if ($scope.createTime != "")
        ...
        $http.get(url).success(function (pagedata) {
            $scope.subscribeBills = pagedata.data.content;
    ...
    $scope.exportData = function (param){
        if (param == 'all') { // 全部导出到excel表中
            var url = 'subscribe-bills/collection?1=1';
            if ($scope.createTime != "")
                url += "&createTime="+$scope.createTime;
            if ($scope.region != "")
                url += "&region=" + $scope.region;
            ...
            alasql('SELECT * INTO XLS("认购账单.xls",?) FROM ?', [myStyle, $scope.billDatas]);
        } else {
            var array = new Array();
            array.push(param);
            alasql('SELECT * INTO XLS("认购账单.xls",?) FROM ?', [myStyle, array]); // 单个导出到excel表中
        }
    };
