 ...
 * 用户可以在地图上自定义轨迹运动
 * 可以自定义路过某个点的图片，文字介绍等。
 * 主入口类是<a href="symbols/MyBMapLib.LuShu.html">LuShu</a>，
 * 基于Baidu Map API 1.2。.
 *
...
/**
 * @namespace BMap的所有library类均放在MyBMapLib命名空间下
 */
var MyBMapLib = window.MyBMapLib = MyBMapLib || {};
...
(function() {
    ...
    /**
     * @exports LuShu as MyBMapLib.LuShu
     */
    var LuShu =
        /**
         * LuShu类的构造函数
         ...
         * }<br />.
         * @example <b>参考示例：</b><br />
         * var lushu = new MyBMapLib.LuShu(map,arrPois,{defaultContent:"从北京到天津",landmarkPois:[]});
         */
        MyBMapLib.LuShu = function(map, path, opts) {
    ...
    baidu.object.extend(LuShu.prototype, {
        ...
        _move: function(initPos,targetPos,effect) {
            var me = this;
            //此比列尺要在类里定义，这里只是为了说明，才放到这里。
            // LuShu.scales=...
            var scales=[20,50,100,200,500,1000,2000,5000,10000,20000,25000,50000,100000,200000,500000,1000000,2000000];
            //当前的帧数
            currentCount = 0,
                //步长，米/秒
                timer = me._map.getZoom(),
                step = this._opts.speed *scales[19-timer], //this._oprts.speed改为比例尺倍率，即一秒钟移动多少个比例尺。
                //初始坐标
