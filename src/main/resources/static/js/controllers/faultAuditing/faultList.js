...
app.controller('faultListController', ['$scope', '$http', '$modal', 'toaster', function ($scope, $http, $modal, toaster) {
    $scope.status = "";
    $scope.time = "";
    $scope.plateNumber = "";
    $scope.bikeType = "";
    ...
    function init(){
        $http.get('fault-auditing/fault-status').success(function (data){
            var allStatuses = {
                id : '0',
                status : '全部'
            };
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            ...
            { field: 'bikeType.name', displayName: '车辆类型名称', width:'160px'  },
            ...
            { field: 'plateNumber', displayName: '单车编号', width:'160px'  },
            { field: 'faultType', displayName: '故障类型', width:'160px'  },
            ...
            { field: 'remark', displayName: '备注', width:'160px'  },
            { field: 'faultStatus.status', displayName: '状态', width:'180px' },
            { field: 'time', displayName: '举报时间', width:'160px',cellTemplate: '<div class="ngCellText ng-scope col8 colt8" >{{row.entity.time | date:"yyyy-MM-dd"}}</div>' },
    ...
    $scope.getPagedDataAsync = function (pageSize, page) {
        var url = 'fault-auditing?page=' + page + '&size=' + pageSize ;
    ...
    $scope.seeRowIndex = function(entity){
        var rtn = $modal.open({
            templateUrl: 'tpl/faultAuditing/see_fault.html',
            controller: 'seeFaultController',
            resolve:{
    ...
    $scope.determine = function(entity) {
        if(this.row.entity.faultStatus.id == "2"){
            ...
            $http.put('fault-auditing/determine/' + this.row.entity.id, this.row.entity).success(function (data) {
    ...
    $scope.cancel = function(entity){
        if(this.row.entity.faultStatus.id == "3"){
            ...
            $http.put('fault-auditing/cancel/' + this.row.entity.id, this.row.entity).success(function (data) {
                $scope.pop('success', '', '修改成功！');
                $scope.getPagedDataAsync($scope.pagingOptions.pageSize, $scope.pagingOptions.currentPage);
            }).error(function (err) {
                alert(err.error);
