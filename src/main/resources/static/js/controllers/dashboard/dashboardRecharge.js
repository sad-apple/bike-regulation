...
app.controller('dashBoardRecharge', ['$scope', function ($scope) {
  ...
  // 数字滚动块
  // var dashboardRechargeTotalNum = formatNumber(5000000);//单车充值目标余额oAt
  // console.log(dashboardRechargeTotalNum);
  // var oAt = dashboardRechargeTotalNum.toString();
  // var dashboardRechargePresentNum = formatNumber(4000000);//单车充值当前余额oAp
  // var oAp = dashboardRechargePresentNum.toString();
  // for (var i = -1; i < oAt.length; i++) {
  //   var text = '';
  //   if (i == 0) {
  //     for (var index = 0; index < oAp.length; index++) {
  //       text += '<i>' + oAp[index] + '</i>'
  //     }
  //     $("#dashBoardRechargenumScroll").html(text);
  //   }
  // }
  // setNum('#dashBoardRechargenumScroll i', oAt);

  // threeDou('#dashBoardRechargenumScroll i','i');
...
// 生成账单编号
  ...
  function getRechargeDetailsData() {
    ...
    $scope.tableSource = rechargeDetailsData.reverse();
  }
    ...
    setInterval(function () {
      ...
      $scope.$apply(function () {
        for (var i = 0; i < moneyData.length; i++) {
          moneyData[i].shift();
          if (i == 0) {
            randomNum1 = Math.round(Math.random() * 5);//4
            moneyData[i].push(randomNum1);
          } else {
            randomNum2 = Math.round(Math.random() * 3);//1
